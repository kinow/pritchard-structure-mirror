...
*/
  ...
  printf("");//printf(" I     Iterate to improve tree:  ");
  //printf("Hello");
  //printf("");
  //printf("Hello");
  if (improve) {
    if (nbody)
      ...
      printf("");//printf("Equal-Daylight algorithm\n");
  } else
    printf("No\n");
  if (improve)
    printf("");//printf(" D  Try to avoid label overlap?  %s\n",
           //(labelavoid? "Yes" : "No"));
  printf("");//printf(" S      Scale of branch length:");
  if (rescaled)
    printf("");//printf("  Automatically rescaled\n");
  else
  ...
  printf("");//printf(" C   Relative character height:%8.4f\n", charht);
  if ((((plotter == pict || plotter == mac)  &&
       (((grows == vertical && labelrotation == 0.0) ||
       (grows == horizontal && labelrotation == 90.0))))))
    printf("");//printf(" F                        Font:  %s\n Q" 
   ...
   //     (pictoutline ? "Outlined" : "Unoutlined"));
    ...
    printf("");//printf(" F                        Font:  %s\n",fontname);
  if (plotter == ray) {
    printf("");//printf(" M          Horizontal margins:%6.2f pixels\n", xmargin);
    printf("");//printf(" M            Vertical margins:%6.2f pixels\n", ymargin);
  } else {
    printf("");//printf(" M          Horizontal margins:%6.2f cm\n", xmargin);
    printf("");//printf(" M            Vertical margins:%6.2f cm\n", ymargin);
  }
  printf("");//printf(" #           Page size submenu:  ");
  ...
  if (((int) ((pagex / paperx) + 0.5) == 1) && 
    ...
    printf("");//printf ("one page per tree\n") ;   
  else
...
void plotlabels(char *fontname)
{
  ...
  for (i = 0; i < (nextnode); i++) {
	...
	//printf("Font Color: %f %f %f",r/10,g/10,b/10);
		
		char *fontcolor[(nextnode)];
		double  colormap[(nextnode)][3];
		char *ptr = lp->nayme;
		//char val = ptr[1];
		//printf("Node Name:%s\n",ptr);
		//printf("Node Name:%c\n",val);
		...
		int val = atoi(ptr);
		//printf("Node Name:%i\n",v);
		//printf("Node Name: %s, (x=%f,y=%f) \n",nodep[i]->nayme,nodep[i]->xcoord,nodep[i]->ycoord);
		//printf("Node Name: %s, (x=%f,y=%f) \n",nodep[i]->nayme,xscale * (lp->xcoord + dx + xoffset),yscale * (lp->ycoord + dy + yoffset));
		...
		if (val==1){
			...
			fontcolor[i] = "0.98 0.60 0.0";
		}
		else if (val==8){
			fontcolor[i] = "0.5 0.25 0.37";
		}
		else if (val==9){
			fontcolor[i] = "0.98 0.6 0.37";
		}
		else if (val==10){
			fontcolor[i] = "0.1 0.6 0.75";
		}
		else if(val==11){
			fontcolor[i] = "0.2 0.6 0.5";
		}
		else if (val==12){
			fontcolor[i] = "0.7 0.3 0.75";
		}
		else if (val==13){
			fontcolor[i] = "0.9 0.2 0.75";
		}
		else if (val==14){
			fontcolor[i] = "0.35 0.1 0.75";
		}
		else if (val==15){
			fontcolor[i] = "0.15 0.89 0.30";
		}
		else if (val==16){
			fontcolor[i] = "0.6 0.2 0.75";
		}
		else if (val==17){
			fontcolor[i] = "0.35 0.9 0.4";
		}
		else if (val==18){
			fontcolor[i] = "0.12 0.38 0.15";
		}
		else if (val==19){
			fontcolor[i] = "0.9 0.09 0.23";
		}
		else if (val==20){
			fontcolor[i] = "0.59 0.23 0.11";
		}
		else if (val==21){
			fontcolor[i] = "0.38 0.2 0.95";
		}
		else{
			double r = randint() % 10;
			double g = randint() % 10;
			...
			char str[100];
			sprintf(str, "%f %f %f", (r/10),(g/10),(b/10));
			fontcolor[i] = str;
		}
		...
		*/
	      ...
	      plottext2(lp->nayme, lp->naymlength,
	               labelheight * expand * xscale / compr, compr,
	               xscale * (lp->xcoord + dx + xoffset),
	               yscale * (lp->ycoord + dy + yoffset), -180 * labangle / pi,
	               font,fontname,fontcolor[i]);
...
//function double convert_to_decimal(){
...
void user_loop()
{
  ...
  while (!canbeplotted) {
    ...
    canbeplotted = true;
    if (preview) {
      printf("");//printf("Preview window displayed... press \"Change\" button to return to menu.\n");
...
void setup_environment(int argc, Char *argv[])
{
  ...
  where = root;
  rotate = true;
  printf("");//printf("Tree has been read.\n");
  printf("");//printf("Loading the font ... \n");
  loadfont(font,argv[0]);
  printf("");//printf("Font loaded.\n");
  previewing = false;
  ansi = ANSICRT;
  ibmpc = IBMCRT;
  firstscreens = true;
...
void drawtree(int argc, Char *argv[])
{
//printf("hello World \n");
  long stripedepth;
  ...
  if (!((previewer == winpreview || previewer == xpreview || previewer == mac)
	...
	char fullpath[1024] = "library/neighbor/plots/";
	char *myfilename;
	
	myfilename = strcat(fullpath,argv[1]);
	
	//printf("str: %s\n",argv[1]);
	
	
	//#define MYPLOTFILE argv[1]
	#define MYPLOTFILE myfilename
	
    //openfile(&plotfile, PLOTFILE, "plot file", "wb", argv[0], pltfilename);
	openfile(&plotfile, MYPLOTFILE, "plot file", "wb", argv[0], pltfilename);
  ...
  if (plotter == pict && wasplotted){
	...
	retcode=SetFInfo(CtoPstr(MYPLOTFILE),0,&fndrinfo);
#endif
  }
  if (plotter == lw && wasplotted){
	...
	retcode=GetFInfo(CtoPstr(MYPLOTFILE),0,&fndrinfo);
    fndrinfo.fdType='TEXT';
    //retcode=SetFInfo(CtoPstr(PLOTFILE),0,&fndrinfo);
	retcode=SetFInfo(CtoPstr(MYPLOTFILE),0,&fndrinfo);
