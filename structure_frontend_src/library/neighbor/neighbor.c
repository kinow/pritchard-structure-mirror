...
void jointree()
{
  ...
  bk -= av[el[2] - 1];
  if (progress) {
    printf("");//printf("last cycle:\n");
    //putchar(' ');
    nodelabel((boolean)(av[el[0] - 1] > 0.0));
    printf("");//printf(" %ld  (%10.5f) joins ", el[0], bi);
    nodelabel((boolean)(av[el[1] - 1] > 0.0));
    printf("");//printf(" %ld  (%10.5f) joins ", el[1], bj);
...
void maketree()
{
  ...
  if (progress) {
    ...
    if (trout)
      printf("");//printf("Tree written on file \"%s\"\n\n", outtreename);
  }
}  /* maketree */
