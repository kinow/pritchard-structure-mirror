...
void plottext2(Char *pstring,long nchars,double height_,double cmpress2,
                        double x,double y,double slope,short *font_,char *fontname,char *fontcolor)
{
#ifdef max
#undef max
#endif
...
#define min4(a,b,c,d) (min(min(a,b),min(c,d)))
 ...
 /* print native font.  idraw, PS, pict, and fig. */
    ...
    if (rectintersects(px0,py0,px1,py1,clipx0,clipy0,clipx1,clipy1)) {
	  ...
	  //add by mike
	  //fprintf("test","%f %f translate %f rotate\n",x-(clipx0*xunitspercm),y-(clipy0*xunitspercm),-slope);
