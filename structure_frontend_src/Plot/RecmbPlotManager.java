...
public class RecmbPlotManager {
	...
	public RecmbPlotManager() {
	}

	public void plot(double[][] plotdata, String title) {

		stg = new Plot(plotdata[0], "Rec", "Recombination Rate Vs. Iterations");
		...
		target = new JFrame("Structure Plotting: Recombination Rate - " + title);
	...
	public void plot(AppProc proc) {
		...
		final Plot g = new Plot(proc, "Rec", 0,
				"Recombination Rate Vs. Iterations");
		g.start();
		JPanel p = new JPanel();
		p.setLayout(new GridLayout(0, 1));
		p.add(g);
		...
		target = new JFrame("Structure Plotting: Recombination Rate");
		target.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				g.stop();
				target.setVisible(false);
	...
	public void plotHist(double[][] data, String title) {
		...
		HistPlot hist = new HistPlot(data[0], "Recombination rate", "Histogram");
		...
		target = new JFrame(
				"Histogram of distribution of recombination rate - " + title);
		target.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				target.setVisible(false);
			}
