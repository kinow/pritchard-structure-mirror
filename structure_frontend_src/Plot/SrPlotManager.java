...
public class SrPlotManager {
	...
	public SrPlotManager() {
	}

	public void plot(double[][] plotdata, String title) {

		stg = new Plot(plotdata[0], "r", "r Vs. Iterations");
		JPanel p = new JPanel();
		...
		target = new JFrame("Structure Plotting: r - " + title);
	...
	public void plot(AppProc proc) {
		...
		final Plot g = new Plot(proc, "r", 1, "r Vs. Iterations");
		g.start();
		...
		target = new JFrame("Structure Plotting: r");
		target.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				g.stop();
				target.setVisible(false);
	...
	public void plotHist(double[][] data, String title) {
		...
		HistPlot hist = new HistPlot(data[0], "r", "Histogram");
		JPanel p = new JPanel();
		p.setLayout(new GridLayout(0, 1));
		...
		target = new JFrame("Histogram of distribution of r - " + title);
		target.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				target.setVisible(false);
			}
