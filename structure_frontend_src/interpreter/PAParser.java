...
public class PAParser implements PAParserConstants {
	...
	final public void parse(PAContext context) throws ParseException {
		...
		label_1: while (true) {
			...
			case STRING_LITERAL:
				...
				try {
					context.engine.process(x.image.substring(1, x.image
							.length() - 1));
				} catch (PainterException e) {
					{
						if (true) {
			...
			case IDENTIFIER:
				...
				try {
					context.engine.process(new PAToken(x.image,
							PAToken.IDENTIFIER));
				} catch (PainterException e) {
					{
						if (true) {
			...
			case KEY_IDENTIFIER:
				...
				try {
					context.engine.process(new PAToken(x.image.substring(1,
							x.image.length()), PAToken.KEY));
				} catch (PainterException e) {
					{
						if (true) {
							throw new ParseException(e.toString());
			...
			case IMMEDIATE_IDENTIFIER:
				...
				try {
					context.engine.process(new PAToken(x.image.substring(2,
							x.image.length()), PAToken.IMMEDIATE));
				} catch (PainterException e) {
					{
						if (true) {
							throw new ParseException(e.toString());
			...
			case LBRACE:
				...
				try {
					context.engine.process(new PAToken(null,
							PAToken.START_PROCEDURE));
				} catch (PainterException e) {
					{
						if (true) {
							throw new ParseException(e.toString());
			...
			case RBRACKET:
				...
				try {
					context.engine
							.process(new PAToken(null, PAToken.END_ARRAY));
				} catch (PainterException e) {
					{
						if (true) {
							throw new ParseException(e.toString());
